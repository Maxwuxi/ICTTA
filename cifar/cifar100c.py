...
def create_imbalanced_dataset(x, y, total_samples, num_classes):
    ...
    weights = (np.array(range(num_classes)) + 1) / num_classes
    samples_per_class = (weights * total_samples / num_classes).astype(int)
...
def evaluate(description):
    ...
    for severity in cfg.CORRUPTION.SEVERITY:
        for i_c, corruption_type in enumerate(cfg.CORRUPTION.TYPE):
            ...
            x_test1, y_test1 = create_imbalanced_dataset(x_test, y_test,cfg.CORRUPTION.NUM_EX,100)
            #100 num_classes
            #print(len(x_test))
            metrics = compute_metrics(model, x_test1, y_test1,100,cfg.TEST.BATCH_SIZE)
