...
class CoTTA(nn.Module):
    ...
    def forward_and_adapt(self, x, model, optimizer):
        ...
        # Student update
        loss = (ourloss(outputs, outputs_ema)).mean(0)
...
def ourloss(x, x_ema):
    ...
    entropy=-a1*(x_ema.softmax(1) * x.log_softmax(1)).sum(1)
    regularization=a2*((weight)*(x_ema.softmax(1) * x.log_softmax(1))).sum(1)
